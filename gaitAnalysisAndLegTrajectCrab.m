...
vf =0;
x0 =-L/2;
xf = L/2; % should be at stride length at end of traject

[xa0,xa1,xa2,xa3] = cubicTrajectConsts(t0,t4,x0,xf,v0,vf);
...
for i = 1:length(timeMat)
    ...
    [xval, dxval] = cubicTrajectEqn(xa0,xa1,xa2,xa3, t);
    % row 1 is y position, row 2 is z position
    posF_g(:,i) = [xval;maxFootH*sin((w*xval)+(pi/2))];
    velF_g(:,i) = [dxval;maxFootH*w*dxval*cos(w*xval)];
end
...
for i=1:numLegs % for all 4 legs
    for j=1:length(timeMat) % time discrete
        Alpha(i,j)=(atan2(xf_H(i,j),yf_H(i,j)));
...
sideLims = [-8,16;-8,8;0,8];

animateWalk(time,cycleTimeJntPosLeg1,cycleTimeJntPosLeg2,cycleTimeJntPosLeg3,cycleTimeJntPosLeg4, 'Walk side 1 Cycle', 'Walk side 1 Cycle.gif',sideLims)
